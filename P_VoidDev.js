...
// ── ADMIN AUTH ────────────────────────────────────────────────
const ADMIN_PASSWORD = "voiddev2026";
...
function updateAdminStatus() {
    ...
    if (isAdmin) {
        if (indicator) {
            indicator.innerHTML = '✓ Admin Mode Active <span style="font-size:12px;">(Click to Logout)</span>';
        ...
        if (indicator) {
            indicator.innerHTML = '🔒 Not Admin <span style="font-size:12px;">(Click to Login)</span>';
...
const PINNED_PROJECTS = [
    {
        ...
        pinned: true,
        name: 'Void Media',
        description: 'A social messaging platform built entirely by Team VoidDev. Users can publish posts, follow a live feed, manage friends, receive notifications, and upload media — all in a sleek, dark interface. Deployed on Render and production-ready.',
        technologies: ['Python', 'Flask', 'HTML', 'CSS', 'JavaScript', 'JSON', 'MongoDB', 'Cloudinary', 'Render'],
...
function addProject() {
    ...
    if (!name || !description) { alert('Please fill in name and description.'); return; }
    ...
    // Reset inputs
    ['projectName','projectDescription','projectTechnologies','projectLink','projectImage']
...
function displayProjects() {
    ...
    const userProjects = JSON.parse(localStorage.getItem('voiddevProjects')) || [];
    const all = [...PINNED_PROJECTS, ...userProjects];

    grid.innerHTML = '';

    if (all.length === 0) {
        grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:#7a6a8a;padding:40px;">No projects yet.</p>';
        return;
    }
